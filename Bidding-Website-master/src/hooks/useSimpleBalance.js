...
/**
 * Simple balance hook - Bank transfer only balance management
 */
export const useSimpleBalance = () => {
  ...
  const fetchBalanceInfo = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await simpleBankService.getBalanceInfo();
  ...
  useEffect(() => {
    ...
    window.addEventListener('simpleBalanceUpdate', handleBalanceUpdate);
    
    return () => {
      window.removeEventListener('simpleBalanceUpdate', handleBalanceUpdate);
    };
  }, [fetchBalanceInfo]);
  ...
  // Add balance using simple bank service
  const addBalance = useCallback(async (amount, bankDetails) => {
    try {
      setLoading(true);
      const result = await simpleBankService.addBalance(amount, bankDetails);
      
      // Trigger balance refresh
      await fetchBalanceInfo();
      
      // Dispatch global update event
      window.dispatchEvent(new CustomEvent('simpleBalanceUpdate', {
  ...
  const holdBalance = useCallback(async (amount, productId, bidId) => {
    try {
      const result = await simpleBankService.holdBalance(amount, productId, bidId);
  ...
  return {
    ...
    addBalance,
    holdBalance,
    isConnected: true // Always connected for simple service
  };
};
 ...
 */
export const useSimpleBiddingBalance = () => {
  const balanceHook = useSimpleBalance();
...
export default useSimpleBalance;